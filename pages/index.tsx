...
export const HomePage: React.FC = () => {
  ...
  const handleChange = (_event: React.ChangeEvent<HTMLInputElement>, changeData: InputOnChangeData) => {
    const newFiltered = data?.items
      ? data.items.filter(i => i.metadata.name.toLowerCase().indexOf(changeData.value.toLowerCase()) > -1)
      : [];
    ...
    <div>
      ...
      <Accordion exclusive={false} style={{ fontSize: '20px' }} inverted>
        {filtered.map((i, index) => (
          <span key={index}>
            <Kind active={activeIndex === index} index={index} handleClick={handleClick} crd={i} />
            <hr />
          </span>
        ))}
      </Accordion>
